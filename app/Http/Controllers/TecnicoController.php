...
class TecnicoController extends Controller
{
    ...
    public function storeAnexo2(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'equipos.*.equipment_id'       => 'required|exists:equipment,id',
        ...
        // Enriquecer cada item con los datos del equipo del catálogo
        $equiposData = collect($request->equipos)->map(function ($item) {
            $equipo = Equipment::findOrFail($item['equipment_id']);
            return [
                'equipment_id'    => $equipo->id,
                'nombre'          => $equipo->nombre,
                'categoria'       => $equipo->categoria,
                'marca'           => $equipo->nombre,   // compatibilidad con el PDF
        ...
        // El PDF se guardará en la misma ruta almacenada en el contrato
        $this->pdfService->generateAndSave($contract);
